...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        if (!pid) {
            ...
            for (int j = 0; j < n; ++j) {
                if (i != j) {
                    ...
                    ASSERT_SYS_OK(dup2(channels[i][j][0], determine_read(i, j)));
                    //printf("assigned write from %d to %d to %d in process %d\n", i, j, determine_write(i, j), i);
                    ASSERT_SYS_OK(dup2(channels[j][i][1], determine_write(i, j)));
                }
            }
            for (int j = i; j < n; ++j) {
                for (int k = 0; k <= i; ++k) {
                    if (j != k) {
                        ASSERT_SYS_OK(close(channels[j][k][0]));
                        ASSERT_SYS_OK(close(channels[j][k][1]));
                        ASSERT_SYS_OK(close(channels[k][j][0]));
                        ASSERT_SYS_OK(close(channels[k][j][1]));
                    }
                }
            }
        ...
        for (int j = 0; j < i; ++j) {
            ...
            ASSERT_SYS_OK(close(channels[j][i][0]));
            ASSERT_SYS_OK(close(channels[j][i][1]));
